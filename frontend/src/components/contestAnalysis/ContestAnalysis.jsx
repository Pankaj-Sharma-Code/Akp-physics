  ...
  useEffect(() => {
    const fetchQuestionsAndCorrectAnswers = async () => {
      try {
        ...
        const questionResponse = await axios.get(`/api/contests/questions/${contestId}`);
        setQuestions(questionResponse.data.question);
        
        // Fetching results for the test to get the selected answers
        const resultResponse = await axios.get(`/api/contest/answers/result/${contestId}`);
        setResultData(resultResponse.data);

        // Fetching correct answers for the test
        const correctAnswerResponse = await axios.get(`/api/contest/answers/result/getAnswers/${contestId}`);
